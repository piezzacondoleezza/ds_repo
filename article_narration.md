...
**Личные заметки по статье**

Данная статья не столько произвела на меня впечатление по причине огромной проделанной работы ее авторами, а скорее помогла погрузиться во все великолепие мира распределенных баз данных и понять, насколько огромное количество интересных проблем приходится решать людям, пытающимися разработать что-то в этой сфере. Наибольшее впечатление на меня произвело элегантность и простота решения проблем дедлоков и согласованности реплик. А так же показало многие проблемы, на которые я ранее не обращал внимание при просмотре метрик на работе, такие как пропускная способность сети, которые оказались очень важны для работы механизма базы данных. Многие вещи я читал впервые, и, даже когда сроки кончатся, я с удовольствием перечитаю данную статью еще более внимательно, уже не пытаясь понять и сформулировать, что же происходит внутри, а более детально разобраться с предоставленными авторами примерами и изучить опыт людей в данной сфере. Я всегда пользовался готовыми решениями в области баз данных и действительно не задумывался, с каким количеством различных трудностей приходится сталкиваться при разработке решений. 

Из преимуществ хочется выделить, что данное решение безусловно может найти свое применение в мире. Предположительно, ее применение будет очень разумно и в сервисах, которым очень важна сохранность данных, и в сервисах, которые стараются выигрывать как можно больше по latency. В данной статье были представлены очень разумные оптимизации репликации по latency(асинхронные и синхронные опции подсчета), что как раз таки позволяет выбрать удобное для каждого, в зависимости от его собственных нужд. Так же очень хорошо выглядит идея пожертвовать строгим порядком в угоду оптимизации по времени для части с диском.

Однако из недостатков хочется заметить следующую вещь, не очень понятно как данную базу данных поддерживать, кажется, что с точки зрения реализации это очень тяжелый проект, и не совсем тривиальным будет понять, как это будет выглядеть в реальной жизни, и существует ли в принципе какое-либо простое решение данной проблемы. Многие вещи, такие как умные lock'и, вызывают некоторые вопросы вида: "как это будет выглядеть под капотом?", на которые в данной статье, к сожалению, я не нашел ответа. Так же хочется заметить, что возможно сервисам, которым очень важно экономить память(например, людям, которые пишут софт для различных чипов) это может не подойти, ибо памяти все же задействуется немало. Формулируя все это более общно, хочется сравнить это с некоторыми алгоритмами, которые скорее выглядят как интересный абстрактный вариант. В дополнение, хотелось бы увидеть некоторые benchmark'и не на стандартных тестах, а на реальном high-rps сервисе, чтобы убедиться в отсутствии проблем, а так же оценить насколько данный код будет масштабируем не только в плане подстраивания под другие базы данных, а еще и с точки зрения кода и добавления новых интересных фичей. К сожалению, я не изучал данный вопрос, и мои заметки являются лишь результатом прочтения статьи, а не глубокого погружения в тему, что, конечно же, хочется исправить.

Однако, поскольку на теоретические результаты смотреть всегда интересно, все недостатки являются лишь следствием того, что результат довольно новый и Calvin является очень многофункциональным результатом, который вполне может быть пригоден для использования в разработке, а так же впечатляет, как с помощью жонглирования некоторыми результатами полученными на основе использования других баз данных, была получена модель, исправляющая многие недостатки других баз данных, но в то же время использующая все их преимущества. 
